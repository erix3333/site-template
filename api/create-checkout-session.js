...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Resolve your site origin for success/cancel
    const origin =
      (req.headers['x-forwarded-proto'] && req.headers['x-forwarded-host'])
        ? `${req.headers['x-forwarded-proto']}://${req.headers['x-forwarded-host']}`
        : (process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : 'http://localhost:3000');

    // Load catalog from our own API so Admin changes apply
    const products = await fetch(`${origin}/api/catalog`, { cache: 'no-store' }).then(r => {
    ...
    const line_items = items.map(({ id, qty }) => {
      ...
      // If you prefer pre-created Stripe Price IDs, you can switch to: { price: p.stripePrice, quantity }
      return {
        quantity,
        price_data: {
          currency: 'eur',
